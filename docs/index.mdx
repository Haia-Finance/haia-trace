...
<Note>
  ...
  redacted events into `.trace/events`. The shipped template's last stage is
  closed by a business event no adapter produces yet, so a captured payment
  assembles as `partial`. See [Connect x402](/sdk/x402).
</Note>
