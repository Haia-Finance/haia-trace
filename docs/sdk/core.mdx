...
any one of its events closes the stage — and a witness may carry a `where` the
event has to satisfy.
...
`where` is a mapping of top-level payload fields to scalars, all of which must
hold, compared with strict equality — a field the event does not carry never
matches. The key `role` is the one exception: it reads the event's `role`, the
observing side the adapter stamped on it, not a payload field. An `exceptions`
entry takes the same `{ event, where }` mapping, so one
event type can be a milestone under one outcome and a fault under another; a bare
event type faults on every occurrence. See
[`haia-trace template`](/cli/template) for the shape in full.
