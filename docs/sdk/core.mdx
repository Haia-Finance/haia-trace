...
An adapter that records straight to one sink binds it at construction and uses
`record` — stamp and write in one call, no way to mint an event and forget to
persist it. It returns the writer's verdict (see the sink contract below):

```ts
const recorder = createRecorder({ adapter: "my-backend", writer });

recorder.record({ event_type: "my.fact.observed", payload: {} }); // boolean
```
...
<Note>
  Recording nothing is the correct outcome when the disk refuses, but it is only
  a *visible* one if you ask for it — so wire `onError` on any producer whose run
  file matters. Without a handler the refusal has nowhere to go, and the run is
  simply absent.
</Note>
