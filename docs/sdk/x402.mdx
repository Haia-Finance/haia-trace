...
[`haia-trace build`](/cli/build) reads from, so neither side has to be told where
the run lives. That path is relative to the working directory, which is what an
app started from its own project root wants; a process whose cwd is not fixed
should pass `dir` explicitly. Without a `writer`, events go to stdout as NDJSON —
the same encoding, so it can simply be piped into a file.
