...
Templates are per-role, and every witness in them names, through
`where: { role: ... }`, the role that has to have observed it. Apply the one matching the side you traced — one build per side,
