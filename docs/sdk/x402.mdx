...
`.trace/events` is where [`haia-trace build`](/cli/build) reads from unless its
[`--dir`](/cli/overview) says otherwise, so naming it here is what makes the two
sides meet. The writer takes the directory explicitly rather than defaulting to
one — a default would be a filesystem convention two packages had to keep
...
pass an absolute path. Without a `writer`, events go to stdout as NDJSON — the
same encoding, so it can simply be piped into a file. A run can also be mirrored to a Haia
Control Plane project while it is written locally — see
[Mirror events to the Control Plane](/sdk/core#mirror-events-to-the-control-plane).
...
would do — but if you also [mirror the events to a Control Plane
project](/sdk/core#mirror-events-to-the-control-plane), that project holds many
runs side by side and groups them by `context_id`, and there ids that restarted
with each run would fold unrelated payments into one operation.
...
<Note>
  ...
  The shipped templates are per-role — assemble a capture with the one for the
  side that recorded it: `x402-buyer` for a client, `x402-seller` for a resource
  server, `x402-facilitator` for a facilitator. Each matches only events that
  side's hooks record *and* only when its own `role` observed them, so a run
  that traced two sides still yields one honest receipt per side. Every buyer
  stage has a witness in each client kind's flow (HTTP, MCP, and the bare
  `x402Client`), so a clean payment assembles as `full` whichever side you look
  from.

  The server and the facilitator share the verify/settle vocabulary exactly,
  which is why the role constraint matters — but not how a decline reads. The
  SDK hands a facilitator its clean "not valid" through the failure hook, so the
  reason arrives in `error.message`; a resource server gets it as a result, so
