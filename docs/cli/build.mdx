...
`op-1`, `op-2`, …). Folding two runs together would merge two unrelated payments
that happen to share an id, so each run is read, assembled and reported on its
own, and its receipts are stored under its run id.
...
unique within its run: a flat list can hold two different payments both called
`op-1`. `--status` applies here too — `runs[].receipts` holds only the matching
