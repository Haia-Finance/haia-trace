...
[`build`](/cli/build) and [`sample`](/cli/sample) assemble against. Three ship
with the CLI — `x402-buyer`, `x402-seller` and `x402-facilitator`, one per side
of an x402 payment; any other operation needs one you write.
